...
Phase 5AG validated a bounded Hermes PRD-review pilot using bounded explicit context from the PRD, changelog, pilot-mode constraints, security model, and model-provider plan through the validated `local_summary` path. Guardrails held, Hermes used only the localhost adapter and dummy local API key, platform tools remained disabled, the adapter selected `gemma4:26b`, and the pilot produced usable PRD-review findings without external integrations or Desktop actions. Hermes Desktop remains fail-closed because the official DMG verifies as a disk image, but the mounted app remains a setup/bootstrap bundle with invalid strict code-signature behavior and no confirmed trusted final Desktop runtime path.

Repository status is maintained by git; this PRD records Phase 5AG as complete within the foreground-only/no-background-service/no-persistent-Hermes-config/no-external-integration scope. The explicit-context plus `local_summary` prompt shape remains the baseline for bounded local Hermes pilot reasoning.
...
Phase 5AH should be documentation-only: clarify the success criteria and approval triggers for re-enabling disabled Hermes authorities such as shell execution, file edits, gateway behavior, integrations, Desktop launch, and resident mode; also formalize credential-rotation requirements before any resumed Agent Bus reads or writes. Hermes Desktop must remain fail-closed until a later phase resolves the Desktop bootstrap/signature/openability issue and explicitly approves another launch/configuration attempt. Recommended Desktop follow-up remains sending or otherwise acting on the prepared support clarification only after explicit approval. Do not start background services, expose the adapter externally, use cloud providers, send sensitive prompts, connect external integrations, grant shell or file-edit authority, or broaden Hermes authority without a new explicit phase approval.
