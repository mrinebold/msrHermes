...
Phase DESKTOP-8 planned official Hermes Desktop artifact reacquisition only. Hermes Desktop remains fail-closed because the installed app is not trusted as the final Desktop runtime, and no new download, install, launch, replacement, quarantine change, credential setup, permission grant, or external connection has been approved.

Repository status is maintained by git; this PRD records Phase DESKTOP-8 as planning-only and complete within the approved no-download/no-launch/no-replacement scope.
...
Phase DESKTOP-6 confirmed the downloaded DMGs are valid and identical, but Codex cannot mount the APFS DMG for mounted-bundle comparison. Phase DESKTOP-7A reconfirmed the installed bundle is not healthy enough for launch retry and found a pre-existing `Hermes-Setup` process. Phase DESKTOP-8 planned safe official artifact reacquisition and later replacement guardrails only. The next Desktop phase should be DESKTOP-9: download/verification-only for the official artifact, or a narrower unsandboxed manual comparison phase if the existing DMG can be mounted by the user outside Codex. Do not install, replace, launch, remove quarantine, sign in, grant permissions, connect integrations, kill `Hermes-Setup`, or modify Hermes CLI config until a later explicit phase approves it.
