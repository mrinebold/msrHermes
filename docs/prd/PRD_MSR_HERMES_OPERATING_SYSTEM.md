...
Phase 5AB-AC added managed foreground-only pilot infrastructure: a localhost model-router adapter runner and a locked-down Hermes pilot harness. No live pilot was run. Hermes Desktop remains fail-closed because the official DMG verifies as a disk image, but the mounted app remains a setup/bootstrap bundle with invalid strict code-signature behavior and no confirmed trusted final Desktop runtime path.

Repository status is maintained by git; this PRD records Phase 5AB-AC as infrastructure-only and complete within the no-background-service/no-live-pilot/no-persistent-Hermes-config/no-external-integration scope.
...
Phase 5AB-AC prepared the managed foreground adapter runner and locked-down Hermes pilot harness needed for a limited local reasoning pilot. The pilot harness is isolated, localhost-only, no-tool, sanitized, and output-bounded. No live pilot task was run.
