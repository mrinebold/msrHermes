...
Phase DESKTOP-9 downloaded and verified the official Hermes Desktop macOS artifact only. Hermes Desktop remains fail-closed because the reacquired DMG's mounted `Hermes.app` is still a setup/bootstrap bundle and fails strict code-signature verification plus Gatekeeper assessment.

Repository status is maintained by git; this PRD records Phase DESKTOP-9 as verification-only and complete within the approved no-install/no-launch/no-replacement scope.
...
Phase DESKTOP-6 confirmed the downloaded DMGs are valid and identical, but Codex cannot mount the APFS DMG for mounted-bundle comparison. Phase DESKTOP-7A reconfirmed the installed bundle is not healthy enough for launch retry and found a pre-existing `Hermes-Setup` process. Phase DESKTOP-8 planned safe official artifact reacquisition and later replacement guardrails only. Phase DESKTOP-9 verified the reacquired official DMG and mounted app, but the mounted app still fails strict code-signature verification and Gatekeeper assessment. The next Desktop phase should be DESKTOP-10: official artifact integrity escalation and support/release-channel clarification. Do not install, replace, launch, remove quarantine, sign in, grant permissions, connect integrations, kill `Hermes-Setup`, or modify Hermes CLI config until a later explicit phase approves it.
