...
Phase 5AE validated the first usable controlled Hermes next-action pilot by supplying bounded PRD/changelog excerpts as explicit local context through the validated `local_summary` path. Guardrails held, Hermes used only the localhost adapter and dummy local API key, platform tools remained disabled, the adapter selected `gemma4:26b`, and the pilot produced usable recommendation text without external integrations or Desktop actions. Hermes Desktop remains fail-closed because the official DMG verifies as a disk image, but the mounted app remains a setup/bootstrap bundle with invalid strict code-signature behavior and no confirmed trusted final Desktop runtime path.

Repository status is maintained by git; this PRD records Phase 5AE as complete within the foreground-only/no-background-service/no-persistent-Hermes-config/no-external-integration scope. The explicit-context plus `local_summary` prompt shape is now the baseline for bounded local Hermes pilot reasoning.
...
Phase 5AF should preserve the Phase 5AE explicit-context and `local_summary` baseline, then run one bounded forward-looking pilot that asks for the next recommended Hermes operating-system phase after Phase 5AE. Hermes Desktop must remain fail-closed until a later phase resolves the Desktop bootstrap/signature/openability issue and explicitly approves another launch/configuration attempt. Recommended Desktop follow-up remains sending or otherwise acting on the prepared support clarification only after explicit approval. Do not start background services, expose the adapter externally, use cloud providers, send sensitive prompts, connect external integrations, or broaden Hermes authority without a new explicit phase approval. Also confirm or explicitly defer exposed credential rotation before any additional live Agent Bus reads or writes.
