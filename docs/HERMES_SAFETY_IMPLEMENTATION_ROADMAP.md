...
Phase 6U refines the same read-only status command to report audit file counts, latest audit risk level, approval file counts, latest approval action and expiration, count of currently valid approvals where practical, freeze reason visibility, emergency stop script presence, policy check script presence, and dry-run resident loop presence. It remains read-only and does not create directories, write files, start or stop services, call external systems, run Hermes live, or enable resident mode.
