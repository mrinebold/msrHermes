...
Helio then resolves agent routing against the MSR roster and ACL rules. If Hermes asks for a specific specialist, Helio may accept, reroute, or refuse based on governance.
...
Current source candidates: `chat_messages` for chat history and `agent_messages` for executable directives. Helio should expose a normalized `messages` view/API rather than requiring Hermes to understand both.
...
Known status model from current migrations: `pending -> dispatched -> completed/failed`, with `failed -> dispatched` for retry. Some services also reference active states such as `running` or `in_progress`, so Helio must normalize status before exposing it to Hermes.
...
## No Scaffold Decision

Do not add `services/agent_bus/` yet. The message schema is clear, but the complete Hermes-facing bus contract still needs answers for:

- Whether `conversations` should use `chat_sessions`, a new table, or a Helio abstraction.
- Whether `messages` should wrap `chat_messages`, `agent_messages`, or both.
...
Once those answers are consolidated, Phase 6B can scaffold a Helio bus gateway with dry-run tests and no live Supabase connection by default.
