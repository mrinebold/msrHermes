...
run_check "Tailscale status" bash -lc 'if command -v tailscale >/dev/null 2>&1; then ts_cli="$(command -v tailscale)"; elif [ -x "/Applications/Tailscale.app/Contents/MacOS/Tailscale" ]; then ts_cli="/Applications/Tailscale.app/Contents/MacOS/Tailscale"; else ts_cli=""; fi; if [ -n "$ts_cli" ]; then "$ts_cli" version; printf "\nStatus:\n"; "$ts_cli" status 2>&1 || true; printf "\nIPs:\n"; "$ts_cli" ip 2>&1 || true; printf "\nDNS:\n"; "$ts_cli" dns status 2>&1 | sed -n "1,80p" || true; printf "\nPrefs:\n"; "$ts_cli" debug prefs 2>&1 | grep -E "\"(RouteAll|ExitNodeID|ExitNodeIP|ExitNodeAllowLANAccess|RunSSH|AdvertiseRoutes|AdvertiseServices|WantRunning|LoggedOut)\"" || true; else echo "Tailscale not found"; fi'
